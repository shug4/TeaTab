...
function load() {
  try {
    ...
    // 全タブにカードが一枚もない時のみチュートリアルを追加
    const totalCards = Object.values(state.bookmarks).flat().length;
    if (totalCards === 0) {
...
/* ===================== FAVICON ===================== */
// URLを正規化して返す（スキーム補完）．拡張機能内部・無効URLは null
function toFetchableUrl(url) {
  if (!url) return null;
  let s = String(url).trim();
  if (s.startsWith('chrome://') || s.startsWith('chrome-extension://') || s.startsWith('about:')) return null;  //拡張機能用
  if (!/^https?:\/\//i.test(s)) s = 'https://' + s;
  try { return new URL(s); } catch { return null; }
}

// 同一サイトの再取得を避けるキャッシュ（href -> 解決済みファビコンURL）
const _faviconCache = new Map();

// サイトが <link rel="icon"> 等で宣言しているファビコンのうち最大サイズ（最大256px）のURLを返す．
// 拡張機能ページ・chromeページ（chrome-extension:// 等）は fetch できないため Chrome の favicon API でアイコンを取得する．
// HTMLの取得・解析に失敗した場合は /favicon.ico をフォールバックに使う．無効URLは null．
async function resolveFaviconUrl(url) {
  const u = toFetchableUrl(url);
  if (!u) return null;
  if (_faviconCache.has(u.href)) return _faviconCache.get(u.href);

  const result = `https://www.google.com/s2/favicons?sz=128&domain=${encodeURIComponent(u.hostname)}`;

  _faviconCache.set(u.href, result);
  return result;
}

  // カスタムファビコンをrecentFaviconsの先頭に追加（重複除去・最大6件）
  function addRecentFavicon(dataUrl) {
    if (!dataUrl || !dataUrl.startsWith('data:')) return;
    const list = state.recentFavicons || [];
    const filtered = list.filter(f => f !== dataUrl);
    state.recentFavicons = [dataUrl, ...filtered].slice(0, 6);
    save();
  }
  ...
  function render() {
    ...
    document.body.classList.toggle('supervisor-on', state.supervisorOn);
  ...
  function renderBg() {
    ...
    if (state.bgImage) {
      ...
      document.getElementById('gradient-divider').style.display = 'none';
      const [x, y] = pos.split(' ');
      ...
      document.getElementById('gradient-divider').style.display = 'block';
    }
  }
      ...
      delBtn.addEventListener('click', e => {
        ...
        openConfirm(
          ...
          () => {
            ...
            save(); render(); renderStats(); renderStorageInfo();
          }
        );
      });
  ...
  function makeCard(bm, tabId = state.activeTab) {
    ...
    if (bm.favicon) {
      if (bm.favicon.startsWith('data:')) {
        ...
        // 外部URLのファビコンは読み込み完了まで文字フォールバックを表示しておく
        const placeholder = makeFallback(bm.name);
        card.appendChild(placeholder);
        const img = new Image();
        img.onload = () => {
          if (img.naturalWidth < 16) return; // 極端に小さい（破損・ダミー）画像は文字フォールバックを維持
          const el = document.createElement('img');
          el.className = 'bm-favicon';
          el.src = bm.favicon;
          placeholder.replaceWith(el);
        };
        img.onerror = () => { }; // 失敗時は文字フォールバックのまま
        img.src = bm.favicon;
      }
    } else {
    ...
    delBtn.addEventListener('click', e => {
      ...
      save(); renderGrid(); renderStats(); renderStorageInfo();
    });
    ...
    //URLなしカードは押しても反応しない（見た目でも分かるようにクラスを付与）
    if (!bm.url) card.classList.add('no-url');

    //通常モード時：クリックでＵＲＬを新しいタブで開く（URLなしカードは何もしない）
    card.addEventListener('click', () => {
      if (!state.settingsOn && bm.url) window.open(bm.url, '_blank');
    });
  ...
  function makeEmpty(c, r, tabId = state.activeTab) {
    ...
    if (state.settingsOn) {
      ...
      el.addEventListener('drop', async e => {
        ...
        // カード移動D&D
        if (dragSrc) { moveBm(dragSrc.bm, dragSrc.tabId, c, r, tabId); return; }
        // サイドパネルからのD&D
        let data;
        try { data = JSON.parse(e.dataTransfer.getData('application/json')); } catch { return; }
        if (!data.url) return;
        const list = state.bookmarks[tabId] || [];
        // カード作成時と同じ方法（resolveFaviconUrl）で高画質ファビコンを取得．取得できなければタブのファビコンにフォールバック
        const faviconUrl = (await resolveFaviconUrl(data.url)) || data.favicon || null;
        list.push({ id: uid(), url: data.url, name: data.name || data.url, favicon: faviconUrl, col: c, row: r });
        state.bookmarks[tabId] = list;
        save(); renderGrid(); renderStats(); renderStorageInfo();
        toast('カードを追加しました');
      });
      // 右クリックでペーストメニューを表示する（コピー済みの場合のみ）
        ...
        showContextMenu(e.clientX, e.clientY, [
          {
            label: 'ペースト', action: () => {
              const list = state.bookmarks[tabId] || [];
              list.push({ ...copyBm, id: uid(), col: c, row: r });
              state.bookmarks[tabId] = list;
              save(); renderGrid(); renderStats();
              toast('ペーストしました');
            }
          }
        ]);
  ...
  function openModal(bm, col = 0, row = 0, tabId = state.activeTab) {
    ...
    renderRecentFavicons();
    if (bm && bm.favicon) {
      if (window._customFavicon) {
        updateFaviconUploadUI();
      } else {
        showFaviconPreview(bm.favicon);
      }
    }
  }
  ...
  document.getElementById('modal-url').addEventListener('input', function () {
    ...
    faviconDebounce = setTimeout(async () => {
      const url = this.value.trim();
      const fUrl = await resolveFaviconUrl(url);
      if (fUrl && !window._customFavicon) showFaviconPreview(fUrl);
  ...
  //入力されたURL・表示名でブックマークを追加または更新する（URLは空でも可）
  async function saveBookmark(url, name) {
    //URLがあればスキーム補完，無ければ空のまま（クリックしても何も開かないカード）
    //既にスキーム（chrome-extension:// 等）が付いている場合はそのまま使う
    let fullUrl = url;
    if (fullUrl && !/^[a-z][a-z0-9+.-]*:\/\//i.test(fullUrl)) fullUrl = 'https://' + fullUrl;

    const uploadedFavicon = window._customFavicon || null;
    if (uploadedFavicon) addRecentFavicon(uploadedFavicon);
    const favicon = uploadedFavicon || await resolveFaviconUrl(fullUrl);
    const list = getBms();
    ...
    if (modalTarget) {
      ...
      if (bm) {
        ...
        bm.url = fullUrl;
        bm.name = name;
        if (window._faviconCleared) {
          bm.favicon = await resolveFaviconUrl(fullUrl);
        } else if (window._customFavicon) {
          bm.favicon = window._customFavicon;
        } else if (urlChanged) {
          bm.favicon = await resolveFaviconUrl(fullUrl);
        }
      }
      ...
      if (occ.has(`${col},${row}`)) {
        let found = false;
        const activeTab = state.tabs.find(t => t.id === modalTabId);
        const tabCols = activeTab?.cols || state.cols;
        const tabRows = activeTab?.rows || state.rows;
        outer: for (let r = 0; r < tabRows; r++) for (let c = 0; c < tabCols; c++) {
  ...
  //保存ボタン：表示名のみ必須．URL未入力なら確認のうえURLなしカードを保存する
  document.getElementById('modal-ok').addEventListener('click', () => {
    ...
    if (!url) {
      openConfirm(
        'このカードはクリックしても何も開きません。',
        () => saveBookmark('', name),
        { title: 'URLを入れずに保存しますか？', okLabel: '保存' }
      );
      return;
    }
  ...
  document.getElementById('modal-favicon-upload').addEventListener('change', function () {
    ...
    reader.onload = e => {
      ...
      img.onload = () => {
        ...
        window._customFavicon = canvas.toDataURL('image/png');
        window._faviconCleared = false;
        updateFaviconUploadUI();
      };
  ...
  // faviconアップロードUIを現在の_customFaviconの状態に合わせて更新
  function updateFaviconUploadUI() {
    const p = document.getElementById('modal-favicon-preview');
    p.innerHTML = '';
    if (window._customFavicon) {
      const img = document.createElement('img');
      img.src = window._customFavicon;
      p.appendChild(img);
      p.appendChild(document.createTextNode(' 選択されています'));
      const clearBtn = document.createElement('button');
      clearBtn.className = 'favicon-clear-btn';
      clearBtn.textContent = 'カスタムfaviconを削除';
      clearBtn.title = 'カスタムfaviconを削除';
      clearBtn.addEventListener('click', async () => {
        window._customFavicon = null;
        window._faviconCleared = true;
        document.getElementById('modal-favicon-upload').value = '';
        const url = document.getElementById('modal-url').value.trim();
        const fUrl = await resolveFaviconUrl(url);
        if (fUrl) showFaviconPreview(fUrl);
        else updateFaviconUploadUI();
      });
      p.appendChild(clearBtn);
    }
  }

  // 最近使ったカスタムfaviconの履歴をモーダルに描画する
  function renderRecentFavicons() {
    const list = state.recentFavicons || [];
    let wrap = document.getElementById('modal-recent-favicons');
    if (!wrap) return;
    wrap.innerHTML = '';
    if (list.length === 0) return;
    const label = document.createElement('span');
    label.textContent = '最近使用：';
    label.className = 'recent-favicon-label';
    wrap.appendChild(label);
    list.forEach(dataUrl => {
      const img = document.createElement('img');
      img.src = dataUrl;
      img.className = 'recent-favicon-thumb';
      img.title = 'このfaviconを使用';
      img.addEventListener('click', () => {
        window._customFavicon = dataUrl;
        window._faviconCleared = false;
        updateFaviconUploadUI();
        renderRecentFavicons();
      });
  ...
  document.getElementById('settings-btn').addEventListener('click', () => {
    ...
    if (!state.settingsOn) {
      ...
      document.getElementById('supervisor-panel').classList.remove('open');
    }
    save(); render(); renderStats(); renderStorageInfo();
  });
  ...
  // パネル外のクリック・ドラッグ開始で設定パネルを閉じる（管理パネルの操作では閉じない）
  document.addEventListener('mousedown', e => {
    const panel = document.getElementById('settings-panel');
    if (!panel.classList.contains('open')) return;
    if (panel.contains(e.target)) return;
    if (document.getElementById('panel-btn').contains(e.target)) return;
    // 管理パネル本体およびその開閉ボタンの操作では設定パネルを閉じない
    if (document.getElementById('supervisor-panel').contains(e.target)) return;
    if (document.getElementById('supervisor-panel-btn').contains(e.target)) return;
    panel.classList.remove('open');
  });

  // スーパーバイザーパネルの開閉
  document.getElementById('supervisor-panel-btn').addEventListener('click', () => {
    document.getElementById('supervisor-panel').classList.toggle('open');
  });

  // hex+opacityをCSS変数に反映する共通関数
  function applyColorVar(varName, color, opacity) {
    const r = parseInt(color.slice(1, 3), 16);
    const g = parseInt(color.slice(3, 5), 16);
    const b = parseInt(color.slice(5, 7), 16);
    document.documentElement.style.setProperty(varName, `rgba(${r},${g},${b},${opacity})`);
  }

  // 設定パネルの背景色を反映する
  function applyPanelColor() {
    const color = state.panelColor || '#9b7fef';
    const opacity = state.panelOpacity ?? 0.751;
    applyColorVar('--panel-bg', color, opacity);
    document.getElementById('sv-panel-color').value = color;
    document.getElementById('sv-panel-opacity').value = opacity;
  }

  // ブックマークモーダルの背景色を反映する
  function applyModalColor() {
    const color = state.modalColor || '#9b7fef';
    const opacity = state.modalOpacity ?? 0.998;
    applyColorVar('--modal-bg', color, opacity);
    document.getElementById('sv-modal-color').value = color;
    document.getElementById('sv-modal-opacity').value = opacity;
  }

  // タイマーモーダルの背景色を反映する
  function applyTimerColor() {
    const color = state.timerColor || '#9b7fef';
    const opacity = state.timerOpacity ?? 0.998;
    applyColorVar('--timer-bg', color, opacity);
    document.getElementById('sv-timer-color').value = color;
    document.getElementById('sv-timer-opacity').value = opacity;
  }

  // 検索ボックス・時計・自動操作の表示切り替えを反映する
  function applyVisibilityToggles() {
    const showSearch = state.showSearch ?? true;
    const showClock = state.showClock ?? true;
    ...
    document.getElementById('sv-toggle-search').textContent = showSearch ? '表示' : '非表示';
    document.getElementById('sv-toggle-clock').textContent = showClock ? '表示' : '非表示';
    document.getElementById('sv-toggle-search').style.background = showSearch ? '' : 'rgba(255,100,100,0.2)';
    document.getElementById('sv-toggle-clock').style.background = showClock ? '' : 'rgba(255,100,100,0.2)';
  }

  // タブ数・カード数を表示する
  function renderStats() {
    const tabCount = state.tabs.length;
    const cardCount = Object.values(state.bookmarks).flat().length;
    document.getElementById('sv-stats').innerHTML =
      `<div class="sv-stat-row"><span>タブ数</span><span>${tabCount}</span></div>
     <div class="sv-stat-row"><span>全カード数</span><span>${cardCount}</span></div>`;
  }

  // localStorage使用量を計算して表示する
  function renderStorageInfo() {
    const used = Object.keys(localStorage).reduce((acc, key) => {
      return acc + (localStorage.getItem(key)?.length || 0) * 2; // UTF-16で2バイト/文字
    }, 0);
    ...
    const bar = document.getElementById('sv-storage-bar');
    const text = document.getElementById('sv-storage-text');
    bar.style.width = `${pct}%`;
    bar.style.background = pct > 80 ? 'rgba(255,100,100,0.85)' : 'rgba(100,200,255,0.85)';
    text.textContent = `${usedKb} KB / ${limitKb} KB (${pct.toFixed(1)}%)`;
  }

  // カードサイズ系CSS変数を反映する
  function applyCardSizes() {
    const faviconSize = state.cardFaviconSize ?? 32;
    const fontSize = state.cardFontSize ?? 11;
    document.documentElement.style.setProperty('--card-favicon-size', `${faviconSize}px`);
    document.documentElement.style.setProperty('--card-font-size', `${fontSize}px`);
    document.getElementById('sv-favicon-size').value = faviconSize;
    document.getElementById('sv-font-size').value = fontSize;
  }

  document.getElementById('sv-favicon-size').addEventListener('input', function () {
    state.cardFaviconSize = parseInt(this.value);
    applyCardSizes();
  });

  document.getElementById('sv-favicon-size').addEventListener('change', () => save());

  document.getElementById('sv-font-size').addEventListener('input', function () {
    state.cardFontSize = parseFloat(this.value);
    applyCardSizes();
  });

  document.getElementById('sv-font-size').addEventListener('change', () => save());

  // 設定パネル色変更
  document.getElementById('sv-panel-color').addEventListener('input', function () {
    state.panelColor = this.value;
    applyPanelColor();
  });
  document.getElementById('sv-panel-color').addEventListener('change', () => save());

  document.getElementById('sv-panel-opacity').addEventListener('input', function () {
    state.panelOpacity = parseFloat(this.value);
    applyPanelColor();
  });
  document.getElementById('sv-panel-opacity').addEventListener('change', () => save());

  // ブックマークモーダル色変更
  document.getElementById('sv-modal-color').addEventListener('input', function () {
    state.modalColor = this.value;
    applyModalColor();
  });
  document.getElementById('sv-modal-color').addEventListener('change', () => save());

  document.getElementById('sv-modal-opacity').addEventListener('input', function () {
    state.modalOpacity = parseFloat(this.value);
    applyModalColor();
  });
  document.getElementById('sv-modal-opacity').addEventListener('change', () => save());

  // タイマーモーダル色変更
  document.getElementById('sv-timer-color').addEventListener('input', function () {
    state.timerColor = this.value;
    applyTimerColor();
  });
  document.getElementById('sv-timer-color').addEventListener('change', () => save());

  document.getElementById('sv-timer-opacity').addEventListener('input', function () {
    state.timerOpacity = parseFloat(this.value);
    applyTimerColor();
  });
  document.getElementById('sv-timer-opacity').addEventListener('change', () => save());

  // 検索ボックス・時計のトグル
  document.getElementById('sv-toggle-search').addEventListener('click', () => {
    state.showSearch = !(state.showSearch ?? true);
    applyVisibilityToggles();
    save();
  });

  document.getElementById('sv-toggle-clock').addEventListener('click', () => {
    state.showClock = !(state.showClock ?? true);
    applyVisibilityToggles();
    save();
  });

  // 追加インポート（前に追加／後ろに追加）：起動は表示設定パネルのインポートメニューから
  let svImportMode = null; // 'before' or 'after'

  document.getElementById('sv-import-file').addEventListener('change', function () {
    const file = this.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = e => {
      try {
        ...
        if (!imported.tabs || !imported.bookmarks) {
          toast('JSONの形式が正しくありません'); return;
        }

        // インポートするタブのIDが既存と衝突しないようにリマップ
        const idMap = {};
        ...
        Object.keys(imported.bookmarks).forEach(oldId => {
          const newId = idMap[oldId];
          if (!newId) return;
          const bms = imported.bookmarks[oldId];
          bms.forEach(bm => { bm.id = uid(); });
          state.bookmarks[newId] = bms;
        });

        if (svImportMode === 'before') {
          state.tabs = [...imported.tabs, ...state.tabs];
        } else {
          state.tabs = [...state.tabs, ...imported.tabs];
        }

        save(); render(); renderStats(); renderStorageInfo();
        toast(`${imported.tabs.length}件のタブを${svImportMode === 'before' ? '前に' : '後ろに'}追加しました`);
  ...
  // すべてリセットボタン
  document.getElementById('supervisor-reset-btn').addEventListener('click', () => {
    openConfirm(
      'すべてのタブ・ブックマーク・設定を削除します。この操作は元に戻せません。',
      () => {
        localStorage.clear();
        location.reload();
      }
  ...
  //インポートボタン：押すとインポート方法を選ぶメニューを表示する
  // （置換＝全体を上書き / 前に追加・後ろに追加＝現在のタブを残してタブを足す）
  document.getElementById('import-btn-trigger').addEventListener('click', e => {
    const r = e.currentTarget.getBoundingClientRect();
    showContextMenu(r.left, r.bottom + 4, [
      {
        label: '置換インポート', action: () => {
      ...
      },
      {
        label: '前に追加', action: () => {
          svImportMode = 'before';
          document.getElementById('sv-import-file').click();
        }
      },
      {
        label: '後ろに追加', action: () => {
  ...
  document.getElementById('bg-upload').addEventListener('change', function () {
    ...
    reader.onload = e => {
      ...
      // 現在のlocalStorage使用量を計算（bm_bgは新しい画像で置き換わるので除外）
      const currentUsage = Object.keys(localStorage).reduce((acc, key) => {
        if (key === 'bm_bg') return acc;
        return acc + (localStorage.getItem(key)?.length || 0) * 2;
      }, 0);
      const newBgSize = newBg.length * 2;
      const limit = 5 * 1024 * 1024;

      if (currentUsage + newBgSize > limit) {
        const newBgKb = (newBgSize / 1024).toFixed(0);
        const availableKb = ((limit - currentUsage) / 1024).toFixed(0);
        toast(`⚠ 容量不足です（画像: ${newBgKb}KB, 空き: ${availableKb}KB）`);
        this.value = '';
        return;
      }
  ...
  document.getElementById('bg-clear-btn').addEventListener('click', () => {
    ...
    state.bgGradient = null;
    document.getElementById('bg-upload').value = '';
    save(); renderBg(); renderSwatches(); renderStorageInfo();
    toast('背景をクリアしました');
  });
  ...
  /* ===================== CONFIRM ===================== */
  let confirmCallback = null; //確認ダイアログOKボタン押下時に実行するコールバック
  //確認ダイアログを開く（opts.title・opts.okLabelで見出しとOKボタン文言を上書き可能）
  function openConfirm(msg, onOk, opts = {}) {
    confirmCallback = onOk;
    document.getElementById('confirm-deletemsg').textContent = opts.title || 'タブを削除しますか？';
    document.getElementById('confirm-ok').textContent = opts.okLabel || '削除';
  ...
  function openTimerModal(bm, tabId) {
    ...
    document.getElementById('timer-hour-display').textContent = String(t.hour).padStart(2, '0');
    document.getElementById('timer-minute').value = t.minute;
    document.getElementById('timer-minute-display').textContent = String(t.minute).padStart(2, '0');
  ...
  function setupSpinner(id, min, max) {
    const input = document.getElementById(id);
    const display = document.getElementById(`${id}-display`);
    const update = (v) => {
      input.value = v;
      display.textContent = String(v).padStart(2, '0');
    };
    function startHold(delta) {
      const step = () => {
        let v = parseInt(input.value);
        update(v + delta < min ? max : v + delta > max ? min : v + delta);
      };
      step();
      let interval;
    ...
    document.querySelector(`.spinner-up[data-target="${id}"]`).addEventListener('mousedown', () => startHold(1));
    document.querySelector(`.spinner-up[data-target="${id}"]`).addEventListener('touchstart', () => startHold(1));
    document.querySelector(`.spinner-down[data-target="${id}"]`).addEventListener('mousedown', () => startHold(-1));
    document.querySelector(`.spinner-down[data-target="${id}"]`).addEventListener('touchstart', () => startHold(-1));
  }

  setupSpinner('timer-hour', 0, 23);
  setupSpinner('timer-minute', 0, 59);
  ...
  // タイマー発火チェック：定期的に全ブックマークのタイマーを確認し，前回〜今回の間に発火時刻が含まれればURLを開く
  const _firedTimers = new Set();
  let _lastCheckTime = Date.now(); // 前回チェック時刻（ms）

  // 設定パネルのIP入力欄を現在の値で更新する
  function renderTimerIp() {
    document.getElementById('timer-allowed-ip').value = state.timerAllowedIp || '';
  }

  // IP保存ボタン：入力値をstateに保存する
  document.getElementById('timer-ip-save-btn').addEventListener('click', () => {
    state.timerAllowedIp = document.getElementById('timer-allowed-ip').value.trim();
    save();
    toast('IPアドレスを保存しました');
  });

  // 現在のIPを取得してタイマー発火を試みる
  // ローカルIPアドレスをWebRTC経由で取得する
  ...
  async function checkAndFireTimers() {
    const now = new Date();
    const nowMs = now.getTime();
    const prevMs = _lastCheckTime;
    _lastCheckTime = nowMs;

    // prevMs〜nowMs の間に含まれる「分単位の時刻キー」を列挙する
    // Throttlingで間隔が大きく空いても取りこぼさないようにする
    const firedKeys = new Set();
    // 1分ずつ繰り上げてチェック（最大60分分まで）
    const MAX_LOOKBACK_MS = 60 * 60 * 1000;
    const start = Math.max(prevMs, nowMs - MAX_LOOKBACK_MS);
    for (let t = start; t <= nowMs; t += 60 * 1000) {
      const d = new Date(t);
      firedKeys.add(`${d.getDay()}-${d.getHours()}-${d.getMinutes()}`);
    }
    // nowMs自体も必ず含める（ループの端数対策）
    firedKeys.add(`${now.getDay()}-${now.getHours()}-${now.getMinutes()}`);

    // 発火対象のブックマークを収集
    const targets = Object.values(state.bookmarks).flat().filter(bm => {
      if (!bm.timer) return false;
      const key = `${bm.timer.day}-${bm.timer.hour}-${bm.timer.minute}`;
      const fireKey = `${bm.id}-${key}`;
      if (_firedTimers.has(fireKey)) return false;
      return firedKeys.has(key);
    });
    if (targets.length === 0) return;

    // 許可IPが未設定の場合は全許可
    const allowedRaw = (state.timerAllowedIp || '').trim();
    if (!allowedRaw) {
      targets.forEach(bm => {
        const key = `${bm.timer.day}-${bm.timer.hour}-${bm.timer.minute}`;
        _firedTimers.add(`${bm.id}-${key}`);
    ...
    // グローバルIPとローカルIPを並行取得
    const [globalRes, localIps] = await Promise.all([
      fetch('https://api.ipify.org?format=json').then(r => r.json()).catch(() => null),
      getLocalIps(),
    ]);

    const currentIps = [...localIps];
    if (globalRes?.ip) currentIps.push(globalRes.ip);

    // ワイルドカード（*）を正規表現に変換して照合
    function ipMatchesPattern(ip, pattern) {
      const regex = new RegExp('^' + pattern.replace(/\./g, '\\.').replace(/\*/g, '\\d+') + '$');
      return regex.test(ip);
    }

    // いずれかのIPが許可リストのいずれかのパターンに一致すれば発火
    const allowed = currentIps.some(ip => allowedList.some(pattern => ipMatchesPattern(ip, pattern)));
    if (!allowed) return;
  ...
  setInterval(checkAndFireTimers, 4000);
  // タブがアクティブに戻った瞬間にも即チェックする
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') checkAndFireTimers();
  });
  ...
  // スマホ用：時計を1200ms以内に6回連続タップでスーパーバイザーモードON/OFF
  (function () {
    let tapCount = 0;
    let tapTimer = null;
    document.getElementById('clock').addEventListener('touchend', () => {
      ...
      if (tapCount >= 6) {
        tapCount = 0;
        clearTimeout(tapTimer);
        state.supervisorOn = !state.supervisorOn;
        document.body.classList.toggle('supervisor-on', state.supervisorOn);
        if (!state.supervisorOn) document.getElementById('supervisor-panel').classList.remove('open');
        toast(state.supervisorOn ? 'Management Mode ON' : 'Management Mode OFF');
        save(); render(); renderStats(); renderStorageInfo();
      }
    });
  })();
  ...
  document.body.classList.toggle('supervisor-on', !!state.supervisorOn);     //スーパーバイザーモードの初期状態を反映