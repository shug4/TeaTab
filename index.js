...
function makeCard(bm, tabId = state.activeTab) {
  ...
  if (bm.favicon) {
    ...
    img.onload = () => {
      // Google Favicon APIが返す地球儀アイコン16x16pxを削除するため
      // naturalWidthが16以下なら取得失敗とみなして文字フォールバックに切り替える
      if (img.naturalWidth <= 16) img.replaceWith(makeFallback(bm.name));
    };
  ...
  if (modalTarget) {
    ...
    if (bm) {
      ...
      if (window._faviconCleared) {
        bm.favicon = getFaviconUrl(fullUrl);
...
document.getElementById('sv-font-size').addEventListener('input', function () {
  state.cardFontSize = parseInt(this.value);
  applyCardSizes();
});
...
document.getElementById('sv-import-before-btn').addEventListener('click', () => {
  svImportMode = 'before';
  document.getElementById('sv-import-file').click();
});

document.getElementById('sv-import-after-btn').addEventListener('click', () => {
  svImportMode = 'after';
  document.getElementById('sv-import-file').click();
});
